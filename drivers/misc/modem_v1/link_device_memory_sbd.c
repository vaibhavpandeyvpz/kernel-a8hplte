...
static void print_sbd_config(struct sbd_link_device *sl)
{
	...
	pr_info("mif: SBD_IPC {shmem_base:0x%lX shmem_size:%d}\n",
		(unsigned long)sl->shmem, sl->shmem_size);

	pr_info("mif: SBD_IPC {version:%d num_channels:%d rbps_offset:%d}\n",
	...
	for (i = 0; i < sl->num_channels; i++) {
		...
		pr_info("mif: RB_DESC[%-2d][UL](offset:%d) = "
			"{id:%-2d ch:%-3d dir:%s} "
		...
		pr_info("mif: RB_DESC[%-2d][DL](offset:%d) = "
			"{id:%-2d ch:%-3d dir:%s} "
			"{sbdv_offset:%-5d rb_len:%-3d} "
