...
/* RF EAR DET */
		if (gpio_request(pdata->rf_ear_det_gpio, "RF EAR DET")) {
			pr_err(KERN_ERR "GPIO_RF_EAR_DET GPIO set error!\n");
			return;
		}
		gpio_direction_output(pdata->rf_ear_det_gpio, 0);
#endif
}
...
#ifdef CONFIG_RF_EAR_DET
static void set_rf_ear_det_state(struct sec_jack_info *hi, bool state)
{
	struct sec_jack_platform_data *pdata = hi->pdata;

	if(pdata->rf_ear_det_gpio > 0)
		gpio_set_value(pdata->rf_ear_det_gpio, state); /*Uses RF Ear Detect */
}
#endif
...
static void sec_jack_set_type(struct sec_jack_info *hi, int jack_type)
{
	...
	if (jack_type == hi->cur_jack_type) {
		if ((jack_type != SEC_HEADSET_4POLE)&&(jack_type != SEC_HEADSET_3POLE)&&(jack_type!= SEC_EXTRA_ANT_DEVICE))
	...
	if (jack_type == SEC_HEADSET_4POLE) {
        ...
        if((jack_type != SEC_HEADSET_3POLE)&&(jack_type != SEC_EXTRA_ANT_DEVICE)){
			set_sec_micbias_state(hi, false);
        }
	}
...
static irqreturn_t ant_detect_irq_thread(int irq, void *dev_id)
{
	struct sec_jack_info *hi = dev_id;
	struct sec_jack_platform_data *pdata = hi->pdata;
	unsigned npolarity = !pdata->det_active_high;
	...
	pr_info("%s: detect_irq(%d)\n", __func__,
 		gpio_get_value(pdata->ant_det_gpio) ^ npolarity);
	...
	loopcnt = 0;
	while (true) {
		curr_data = gpio_get_value(pdata->ant_det_gpio);
		if (pre_data == curr_data)
			loopcnt++;
		else
			loopcnt = 0;
		...
		if (loopcnt >= check_loop_cnt) {
			if (!curr_data ^ npolarity) {
				/* jack not detected. */
				//handle_jack_not_inserted(hi);
			    sec_jack_set_type(hi, SEC_EXTRA_ANT_DEVICE);
...
static struct sec_jack_platform_data *sec_jack_populate_dt_pdata(struct device *dev)
{
...
#ifdef CONFIG_RF_EAR_DET
	pdata->rf_ear_det_gpio = of_get_named_gpio(dev->of_node, "earjack-rfeardet-gpio", 0);
	if (pdata->rf_ear_det_gpio < 0) {
		pr_err("%s : can not find the earjack-rfeardet-gpio in the dt\n", __func__);
	} else
		pr_info("%s : earjack-rfeardet-gpio =%d\n", __func__, pdata->rf_ear_det_gpio);
#endif

	pdata->ant_det_gpio = of_get_named_gpio(dev->of_node, "earjack-ant-det-gpio", 0);
	if (pdata->ant_det_gpio < 0) {
		pr_err("%s : can not find the earjack-antdet-gpio in the dt\n", __func__);
	} else
		pr_info("%s : earjack-ant-det-gpio =%d\n", __func__, pdata->ant_det_gpio);
	...
	for( i=0; i<6; i++)
	{
		...
		of_parse_phandle_with_args(dev->of_node, "fac-det-zones-list","#list-det-cells", i, &args);
#else
		...
		if(args.args[3]==2)			
			pdata->jack_zones[i].jack_type = SEC_EXTRA_ANT_DEVICE;
		else
	...
	struct device_attribute *attr, char *buf)
{
	...
	val[4] = hi->pdata->jack_zones[4].adc_high;
	val[5] = hi->pdata->jack_zones[5].adc_high;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	token = strsep(&buffer, " ");
	if(kstrtoint(token,0,&val[4]) != 0)
		return -EINVAL;

	token = strsep(&buffer, " ");
	if(kstrtoint(token,0,&val[5]) != 0)
		return -EINVAL;
	...
	hi->pdata->jack_zones[4].adc_high = val[4];
	hi->pdata->jack_zones[5].adc_high = val[5];
...
#if defined (CONFIG_EARJACK_ADC_SYSFS)
	if (device_create_file(jack_dev, &dev_attr_jack_adc) < 0)
		pr_err("Failed to create device file (%s)!\n",
...
#if defined (CONFIG_EARJACK_ADC_SYSFS)
	device_remove_file(jack_dev, &dev_attr_jack_adc);
	device_remove_file(jack_dev, &dev_attr_send_end_btn_adc);
	device_remove_file(jack_dev, &dev_attr_vol_up_btn_adc);
	device_remove_file(jack_dev, &dev_attr_vol_down_btn_adc);
#endif
	device_destroy(jack_class, 0);
	class_destroy(jack_class);
}

#ifdef CONFIG_SEC_USE_EXT_CODEC_BIAS
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	ret = gpio_request(pdata->ant_det_gpio, "ant_detect");
	if (ret) {
		pr_err("%s : gpio_request failed for %d\n",
		       __func__, pdata->ant_det_gpio);
		goto err_gpio_request;
	...
	hi->ant_irq = gpio_to_irq(pdata->ant_det_gpio);
	...
	ret = request_threaded_irq(hi->ant_irq, NULL,
				   ant_detect_irq_thread,
				   IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING |
				   IRQF_ONESHOT, "sec_ant_detect", hi);
	if (ret) {
		pr_err("%s : Failed to ant_irq.\n", __func__);
		goto err_request_detect_irq;
	}
	...
	ret = enable_irq_wake(hi->ant_irq);
	if (ret) {
		pr_err("%s : Failed to enable_ant_irq_wake.\n", __func__);
		goto err_enable_irq_wake;
	}
...
static int sec_jack_remove(struct platform_device *pdev)
{
	...
	gpio_free(hi->pdata->ant_det_gpio);
	iio_channel_release(hi->padc);
	kfree(hi);
	atomic_set(&instantiated, 0);
